...
const translateSingle = async (text: string, cacheSuffix: string, config: PipelineRuntimeConfig, ctx: RunCtx, fullText?: string): Promise<string> => {
  ...
  const optionalFields = ["useCache", "apiKey", "region", "url", "model", "apiVersion", "folderId", "temperature", "maxTokens", "systemPrompt", "userPrompt", "sendSystemPrompt", "useRelay", "relayBase", "domains", "extraBody"] as const satisfies readonly (keyof TranslateTextParams)[];
